    ...
    def build_user_message(
        ...
        if self._knowledge_builder is not None:
            ...
            if knowledge:
                knowledge = f"\n{knowledge}\n"
        instruction = (
            "\n\n请根据以上所有信息，输出一个JSON对象，包含五个字段：\n"
            '1. "analysis": 80-150字的战略局势分析（阵容优劣势/克制关系/当前阶段/应该采取的策略方向）\n'
            '2. "command": 1条15-30字的极简战术指令，不要与上一条战术指令重复\n'
            '3. "fight": 15-30字的团战思路，说明如何保命、输出或站位，避免泛泛说"注意站位"\n'
            '4. "item": 5-10字的出装建议（针对对方关键英雄的克制装备或补全阵容短板）\n'
            '5. "speech_level": 只能是 "brief" 或 "full"。局势方向明显变化、关键克制首次出现、阶段转换或重大团战结果改变策略时用 "full"，其余情况用 "brief"。\n'
            "机制参考仅是候选，需结合局势、英雄、分路和已有装备，"
            "不要重复推荐已购装备。\n"
            "只输出JSON本身，不要添加```json标记或任何其他文字。\n"
            "确保JSON合法可解析，analysis、command、fight和item内的文本不要包含未转义的双引号。"
        )
        return (
